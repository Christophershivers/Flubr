        ...
        public async Task<IActionResult> GetIndex()
        {
            ...
            if (val1 == true)
            {
                ...
                List<PostUserModel> post = _FC.UserPosts.Join(_FC.Followers, post => post.Id, follow => follow.FollowingId, (post, follow) => new { post, follow }).Where(o => o.follow.UserId == _UM.GetUserAsync(User).Result.Id)
                    ...
                    .Select(s => new PostUserModel { Post = s.pos.post.Post, Date = s.pos.post.Date, FirstName = s.use.FirstName, LastName = s.use.LastName, ProfileImage = s.use.ProfileImage, UserName = s.use.UserName, Id = s.use.Id }).ToList();
