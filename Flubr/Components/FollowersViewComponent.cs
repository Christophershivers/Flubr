...
namespace Flubr.Components
{
    public class FollowersViewComponent: ViewComponent
    {
        ...
        public FollowersViewComponent(ILogger<FollowersViewComponent> logger, FlubrContext FC, UserManager<ApplicationUser> UM, SignalHub Sig, MessagesRepository MP)
        {
            _logger = logger;
            _FC = FC;
            _UM = UM;
        ...
        public IViewComponentResult Invoke(string id, int followers)
        {
            ...
            List<ViewFollowersViewModel> myFollowers = _FC.Followers.Where(o => o.FollowingId == id).Join(_FC.Users, followers => followers.UserId, users => users.Id, (followers, users) => new { followers, users }).Select(s => new ViewFollowersViewModel { FirstName = s.users.FirstName, LastName = s.users.LastName, ProfileImage = s.users.ProfileImage }).Take(9).ToList();
